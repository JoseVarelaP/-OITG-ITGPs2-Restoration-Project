					...
					<tr>
						...
						<td>
							...
							<xsl:call-template name="CollapsibleTextTable">
								<xsl:with-param name="title">More Info</xsl:with-param>
								<xsl:with-param name="text">
									The stats on your memory card can fail to load for a few reasons: 
									<ul>
										<li>The Stats.xml file or its digital signatures have become corrupt.</li>
										<li>The Stats.xml file has been tampered with, which is causing it's digital signature check to fail.</li>
									</ul>
									If your saved statistics ever fail to load, your can restore from your "last good" data.  Copy all 3 files in the files <i>LastGood</i> directory into the main profile directory (one level up from <i>LastGood</i>).
								</xsl:with-param>
							</xsl:call-template>
						</td>
					</tr>
